...
- FastAPI
- Uvicorn
- Python 标准库抓取并清洗 Sina A 股公开行情接口
...
task --list       # 查看所有任务
task dev          # 并行启动前后端
task check        # 后端语法检查 + 前端 lint/build
task lint         # 前端 lint
task build        # 前端生产构建
```
...
后端依赖默认安装在项目根目录的 `.venv` 中，不会写入系统 Python 环境。前端依赖位于 `frontend/node_modules`。
